...
- **Fix**: The dashboard status line now correctly says "✓ Up to date" or "✓ No data on card" after a scan finishes and finds no *new* work. Previously, it incorrectly remained stuck on "Waiting for first scan" until the first successful upload occurred during the current boot session. Added `has_probed` flag to `/api/status`.
