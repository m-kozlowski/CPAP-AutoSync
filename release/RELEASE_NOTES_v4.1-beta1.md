...
- **Fix**: SMB uploads now preserve the original file timestamps from the SD card. After each file is successfully written to the NAS, the remote file's `creation_time`, `last_access_time`, `last_write_time`, and `change_time` are set to match the local file's `lastWrite` timestamp, so the backup appears with the correct original date (e.g., the actual sleep-study night) rather than the upload time. Implementation uses `SMB2_SET_INFO` with `FILE_BASIC_INFORMATION` via the existing `libsmb2` raw API (`smb2_cmd_set_info_async`). If the timestamp update fails, the upload itself is still considered successful — a warning is logged but the session continues. One additional SMB round-trip per file (~1–5 ms on a local NAS, negligible next to multi-second file writes). No configuration option needed; enabled unconditionally for all SMB uploads.
