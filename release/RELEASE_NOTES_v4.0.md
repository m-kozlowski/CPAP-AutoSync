# CPAP AutoSync v4.0 — Major Release
...
- **Intuitive Labels**: Replaced technical terms with plain-English descriptions: "What time do you usually finish your therapy session?", "What time will you definitely not go back to sleep?", and "What is the earliest time you go to sleep?".
...
### 🔒 Per-Slider Lock Controls (Mobile-Friendly)
To prevent accidental slider adjustments on mobile devices, each time slider now has an individual padlock toggle.
- **Lock by Default**: All sliders start locked (dimmed at 40% opacity) and disabled to prevent interaction during scrolling.
- **Tap to Unlock**: Click the padlock icon to temporarily enable a slider for adjustment.
- **Auto-Relock on Scroll**: After unlocking, sliders automatically re-lock when you scroll more than 100 pixels away, preventing accidental changes.
- **Touch Action**: CSS `touch-action: pan-y` prioritizes vertical scrolling over horizontal slider drag on touch devices.

### 📝 Clarified User Messages
Several user-facing messages have been rephrased for clarity and accuracy.
- **Smart Start Tooltip**: Updated to clearly explain it's the earliest time the mask is taken off with no intention of putting it back on.
- **Cloud Credentials**: Help text now explicitly states these are NOT your SleepHQ username and password.
- **Danger Zone Warnings**: Force Upload and Reset State warnings are more compact and focus on actual risks without misleading references.
- **Grammar Fixes**: Corrected capitalization consistency ("SD card" vs "SD Card") and removed redundant phrasing ("reinsert it back" → "reinsert it").
...
**Steps:**
   - Download the **`firmware-ota-***.bin`** file from the Releases page (e.g., `firmware-ota-v4.0.bin`). ⚠️ **Do NOT use** `firmware-ota-upgrade-***.bin` — that file is for OTA upgrades only.
   ...
   - Delete any existing rows if needed (by clicking on the "bucket" at the right), then click **Add** once.
   - Ensure the address is set to **`0x0`**.
   - Select the downloaded **`firmware-ota-***.bin`** file.
...
- **Partition Change**: This is a one-time migration. Future v4.x releases will support OTA upgrades from v4.0.
