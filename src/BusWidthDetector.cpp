...
// ============================================================================
// The test runs stealth FIRST (card in CPAP's pristine state), then positive
// control second (to confirm the ISR-masking technique works independently).
//
// Phase 1: TRUE STEALTH — card has CPAP's RCA, we have NOT sent CMD0.
//          Mask ISR, sweep CMD13 to find the RCA, try CMD7+CMD17 to read.
// Phase 2: POSITIVE CONTROL — init card via sdmmc_card_init(), mask ISR,
//          verify bare-metal CMD13 finds the known RCA.
//
// CRITICAL: Phase 1 MUST run before Phase 2, because Phase 2 sends CMD0
//           which destroys the CPAP's card session.
...
void BusWidthDetector::stealthTest() {
    ...
    // The SDMMC peripheral is configured (GPIO, clock at 400kHz, 1-bit).
    ...
    // ════════════════════════════════════════════════════════════════════════
    // Phase 1b: Stealth sector read (only if RCA found)
    // ════════════════════════════════════════════════════════════════════════
    bool readOK = false;
    int detectedBits = 0;
    ...
    if (stealthRCAFound) {
        ...
        if (cardState == 3) {
            ...
            LOG_INFO("===STEALTH=== Card already in Transfer state");
        } else {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // Phase 2: POSITIVE CONTROL — init card ourselves, verify CMD13 works
    // This runs AFTER stealth test since sdmmc_card_init() sends CMD0.
    // ════════════════════════════════════════════════════════════════════════
    ...
    bool controlPass = false;
    uint16_t controlRCA = 0;

    if (err != ESP_OK) {
        LOG_ERRORF("===STEALTH=== Phase 2: Card init FAILED: 0x%x (%s)", err, esp_err_to_name(err));
    } else {
        controlRCA = card.rca;
        LOG_INFOF("===STEALTH=== Phase 2: Card init OK: RCA=0x%04X", controlRCA);

        // Mask ISR again and verify bare-metal CMD13
        SDMMC.intmask.val = 0;
        uint32_t st = 0;
        ...
        if (ok) {
            LOG_INFOF("===STEALTH=== Phase 2 PASS: CMD13(0x%04X) → state=%d(%s) resp=0x%08X",
                      controlRCA, s, SD_STATE_NAME(s), st);
            controlPass = true;
        } else {
            LOG_ERRORF("===STEALTH=== Phase 2 FAIL: CMD13(0x%04X) → resp=0x%08X rintsts=0x%08X",
